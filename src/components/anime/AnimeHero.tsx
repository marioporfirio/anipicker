...
import { translate, genreTranslations, tagTranslations } from '@/lib/translations';
...
export default function AnimeHero({ anime }: AnimeHeroProps) {
  ...
  const mainStudio = anime.studios.nodes.find(studio => studio.name)?.name;
  ...
  const translatedSeason = (season: string, year: number) => {
    ...
    const seasonPT: { [key: string]: string } = {
      'WINTER': 'Inverno',
      'SPRING': 'Primavera',
      'SUMMER': 'Verão',
      'FALL': 'Outono'
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-surface p-4 rounded-lg shadow-md space-y-3 text-sm">
                ...
                {mainStudio && (<div style={{borderColor: accentColor}} className="border-l-4 pl-3"><strong>{language === 'pt' ? 'Estúdio:' : 'Studio:'}</strong> {mainStudio}</div>)}
