                            ...
                            <div className="p-3">
                                ...
                                <div className="max-h-60 overflow-y-auto pr-1">
                                    {customLists.filter(list => list.id !== 'favorites').map(list => (
                                        <div key={list.id} className={`w-full text-left rounded-md font-semibold text-sm transition-colors group ${activeListId === list.id ? 'bg-primary/20' : 'hover:bg-primary/20'}`}>
                                            {confirmDeleteId === list.id ? (
                                                <div className="flex items-center justify-between gap-2 p-2">
                                                    <span className="text-text-secondary text-xs">Apagar &quot;{list.name}&quot;?</span>
                                                    <div className="flex gap-1 shrink-0">
                                                        <button onClick={() => { deleteList(list.id); setConfirmDeleteId(null); }} className="px-2 py-0.5 text-xs bg-red-500/20 text-red-400 rounded hover:bg-red-500/40 transition-colors">Sim</button>
                                                        <button onClick={() => setConfirmDeleteId(null)} className="px-2 py-0.5 text-xs bg-surface text-text-secondary rounded hover:bg-primary/20 transition-colors">Não</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="flex items-center justify-between gap-2 p-2">
